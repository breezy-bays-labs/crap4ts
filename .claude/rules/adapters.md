...
2. **Complexity adapter** — uses `@typescript-eslint/typescript-estree` to parse ASTs. Cyclomatic complexity counts: `IfStatement`, `ConditionalExpression`, `For*Statement`, `While*Statement`, `DoWhileStatement`, `CatchClause`, `LogicalExpression` (`&&`, `||`, `??`), `ChainExpression` (`?.`), and non-default `SwitchCase` nodes with statements (empty fall-throughs are skipped).
